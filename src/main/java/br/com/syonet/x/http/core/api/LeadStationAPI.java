...
import br.com.syonet.x.http.core.api.customer.CustomerAPI;
import br.com.syonet.x.http.core.api.customer.CustomerAPICalls;
...
public class LeadStationAPI {
	private String url;
	private String token;
	private DirUtils dirUtils;
	private boolean useSslSocketFactory = false;

	public LeadStationAPI(String url, String token, DirUtils dirUtils, boolean useSslSocketFactory) {
		this.url = url;
		this.token = token;
	...
	public CustomerAPI customer() {
		return new CustomerAPI(this.build(CustomerAPICalls.class));
	}
	...
	private <R> R build(Class<R> clazz) {
		return APICore.create(clazz, 30000, url, token, useSslSocketFactory);
	}
}
